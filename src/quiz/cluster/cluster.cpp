...
void proximity(const std::vector<std::vector<float>>& points, int id, KdTree* tree, float distanceTol, std::vector<int>& cluster, std::unordered_set<int>& processed){
	if(processed.find(id)==processed.end()){
		processed.insert(id);
		cluster.push_back(id);
		std::vector<int> neighbor_ids=tree->search(points[id],distanceTol);
		for(int neighbor_id: neighbor_ids){
			if(processed.find(neighbor_id)==processed.end()) proximity(points,neighbor_id,tree,distanceTol,cluster,processed);
		}
	}
}
	...
	std::unordered_set<int> processed;
 	for(int id=0;id<points.size();++id){
 		if(processed.find(id)==processed.end()){
 			...
 			proximity(points,id,tree,distanceTol,cluster,processed);
